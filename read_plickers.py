...
def scan_photo( name_photo,group=CLASS_TEST):
    ...
    result =""
    result+='Сканирование завершено\n'
    result+='Результаты:'
    for identification, answer in answers:
        result+=f'Студент: {group[identification - 1]["name"]} {group[identification - 1]["surname"]}, Ответ: {answer}\n'
    #result+=answers
    return result


def scan_video_stream(camera,group=CLASS_TEST):
    ...
    while remaining_students and not stop_scan:
        ...
        if not remaining_students:
            ...
            cv2.destroyAllWindows()
            ans = ""
            #ans+='Сканирование завершено\n'
            #ans+='Результаты:\n'
            #for identification, answer in answers:
            #    ans+=f'Студент: {group[identification - 1]["name"]} {group[identification - 1]["surname"]}, Ответ: {answer}\n'
